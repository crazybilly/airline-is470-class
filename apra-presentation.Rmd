---
title: "A Look at Data Analysis Tools or Why You Should Learn R"
output:
...
## When The Job Is: Creating Complex Prospect Lists
...
#combine both lists then add contact data
my_prospects <- bind_rows( td_around_decatur, td_donors) %>% 
  left_join( 
    demographic_tbl %>% 
...
## Quick Summary Data 
```{r}
boxplot(donordata$totalgiving ~ donordata$roundedage, ylim = c(0,500) )
```
...
- a library (or package) is an optional set of functions 
...
## Everything in R Is Based on Functions

- a function is a command that does something, a verb so to speak
    - `sum( some_numbers, na.rm = T)`
- store results in another object like this:
    - `new_object <- sum( some_numbers, na.rm = T)`
...
# or the same thing, but even more poorly formatted
fullstory  <- bop(scoop(hop(foo_foo, through=forest), up=field_mice), on = head)
```
- really hard to read
...
- https://github.com/crazybilly/airline-is470-class
    ...
    - the fundraising version of this presentation is named "apra-presentation.Rmd"
...
- https://github.com/crazybilly/fundRaising
    ...
    - sample data sets are available here